...
import { SHIELD_MAX_GAP, SHIELD_MIN_GAP } from "@/config/GameConfig";

const SPAWN_LOOKAHEAD = 900;
const DESPAWN_MARGIN = 1200;
const MARGIN_X = 120;

/** Power-up de escudo: mismo patrón de reciclado que LilyPadSpawner. */
export class ShieldPickupSpawner {
  readonly group: Phaser.Physics.Arcade.StaticGroup;
  private pickups: ShieldPickup[] = [];
  ...
  private spawnAt(y: number) {
    ...
    const pickup = new ShieldPickup(this.scene, x, y);
  ...
  update(cameraTopY: number, cameraBottomY: number, time: number) {
    while (this.highestY > cameraTopY - SPAWN_LOOKAHEAD) {
      this.highestY -= Phaser.Math.Between(SHIELD_MIN_GAP, SHIELD_MAX_GAP);
